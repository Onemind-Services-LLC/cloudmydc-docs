...
The full possibilities of this window are described in the [Setting Up Environment](/environment-management/setting-up-environment) article.
...
- **Name** - includes the name (or [alias](/environment-management/environment-aliases) if specified) of the environment and its domain. Using the arrow icon before the environment’s name, you can expand the list of nodes it includes.
...
- **Tags** - displays [Environment Groups](/quickstart/dashboard-guide#environment-groups) and [region](/environment-management/environment-regions/choosing-a-region) of this environment, versions (tags) of the containers, and the name of the deployed project.
- **Usage** - shows the current load (i.e. cloudlets and disk space usage). You can also find the **Billing History** ![Locale Dropdown](./img/DashboardGuide/04-billing-history-icon.png) button here, which leads to a separate tab with [your spending statistics](/account-and-pricing/resource-charging/monitoring-consumed-resources) on the current environment.
...
1. Use the **Set Alias** ![Locale Dropdown](./img/DashboardGuide/06-set-alias-icon.png) icon to provide an [alternative environment name](/account-and-pricing/resource-charging/monitoring-consumed-resources) (domain will remain unchanged).
...
3. Click on **Settings** ![Locale Dropdown](./img/DashboardGuide/08-environment-settings-icon.png) environment settings icon to open a separate tab with numerous [configuration panels](/quickstart/dashboard-guide#environment-settings), check the detailed description in the linked section.
...
- [Clone Environment](/environment-management/cloning-environment)
- [Application Lifecycle Management](/application-setting/application-lifecycle-management)
...
Also, the [Settings](/environment-management/cloning-environment) tab for such an environment will contain only four active options: Collaboration, Change Owner, Migration, and Info.
...
8. Hover over the **Tags** column to manage the [groups of this environment](/quickstart/dashboard-guide#environment-groups) with the **Add/Edit Env Group** buttons ( ![Locale Dropdown](./img/DashboardGuide/15-add-env-group-icon.png) or ![Locale Dropdown](./img/DashboardGuide/16-edit-env-groups-icon.png) respectively).

For the platforms with multiple [regions](/environment-management/environment-regions/choosing-a-region), each environment has a region-dedicated icon in the **Tags** column. It allows you to visually separate instances hosted on the different hardware servers and, upon clicking, shows only environments in the appropriate region.
...
- [Custom Domain Name](/application-setting/domain-name-management/custom-domain-name)
- [Swap Domains](/application-setting/domain-name-management/swap-domains)
- [Application Lifecycle Management](/application-setting/application-lifecycle-management)
...
- [Self-Signed Custom SSL](/application-setting/ssl/self-signed-custom-ssl)
- [Custom SSL](/application-setting/ssl/custom-ssl)

3. In the **SSH Access** section, you can see the Public Keys, SSH Connection, and **SFTP / Direct SSH Access** tabs. The first one allows managing your [public SSH keys](/deployment-tools/ssh/add-ssh-key). The second one shows how to access your environment (either via SSH Gate or Web SSH). The third provides details on the connection over the SFTP/FISH protocols.

![Locale Dropdown](./img/DashboardGuide/19-ssh-access-settings.png)

More info:

- [SSH Gate](/deployment-tools/ssh/ssh-overview)
- [Add SSH Key](/deployment-tools/ssh/add-ssh-key)
- [SSH Access via Web Browser](/deployment-tools/ssh/ssh-access/web-ssh)
- [SSH Access via Local Client](/deployment-tools/ssh/ssh-access/ssh-gate)
- [SSH Protocols](/deployment-tools/ssh/ssh-protocols)
...
More info: [Endpoints](/application-setting/external-access-to-applications/endpoints)
...
More info: [Container Firewall](/application-setting/external-access-to-applications/container-firewall)
...
More info: [Load Alerts](/application-setting/built-in-monitoring/load-alerts)
...
More info: [Automatic Horizontal Scaling](/application-setting/scaling-and-clustering/automatic-horizontal-scaling)
...
More info: [Account Collaboration](/account-and-pricing/accounts-collaboration/collaboration-overview)
...
More info: [Environment Transferring](/environment-management/environment-transferring)

10. Choose **Migration** to move your environment to another set of hardware ([region](/environment-management/environment-regions/choosing-a-region)).
...
More info: [Environment Migration between Regions](/environment-management/environment-regions/migration-between-regions)
...
- [Environment Export](/environment-management/environment-export-and-import/environment-export)
- [Environment Import](/environment-management/environment-export-and-import/environment-import)

12. Switch to the **Info** section to see additional information on the environment _Domain_, its _Owner_ and Creator (can differ due to the [account collaboration](/account-and-pricing/accounts-collaboration/collaboration-overview) feature), [Region](/environment-management/environment-regions/choosing-a-region), and _Creation Date_.
...
Click on the environment in the dashboard to see the list of its [layers](/platform-overview/basics-&-terminology#layer) (load balancers, application servers, databases, etc.). You can further expand these node groups to view and manage separate containers, deployed contexts, attached IP addresses.
...
- Click the **Set Alias** button to configure an [alternative name](/environment-management/environment-aliases) for your layer/node (for example, to define primary and secondary servers in a DB cluster).
...
- Select the **Config** option to open [configuration file manager](/application-setting/configuration-file-manager) that can adjust nodes by [mounting data](/data-storage-container/data-sharing/mount-points), creating/uploading new files, and modifying/removing existing ones.
- Choose the **Log** option to view the log files for the nodes of the layer. A list of [log files](/application-setting/built-in-monitoring/log-files) varies based on the selected instance.
- Click the **Statistics** button to [track the data](/application-setting/built-in-monitoring/statistics) on CPU, RAM, Network, Disk space, and IOPS consumption for a separate node or a set of nodes in real-time.
- Select the **Web SSH** option to connect to your [container over SSH](/deployment-tools/ssh/ssh-access/web-ssh) protocol directly in the browser.
- Use the **Redeploy Container(s)** option to [update nodes](/category/container-deployment) to the preferred tag (version).
- Some nodes can have additional options, such as **Add-Ons** (for installing pluggable modules) or **Remote Desktop** (for [managing Windows-based nodes](/windows-and-.NET/windows-rd-access)).
...
In particular, this feature can be used to create a copy of the environment from another PaaS installation (by [exporting](/environment-management/environment-export-and-import/environment-export) it on one platform and [importing](/environment-management/environment-export-and-import/environment-import) on the other).
...
For the detailed overview, check the appropriate [Environment Import](/environment-management/environment-export-and-import/environment-import) document.
...
- search source (either whole account or the current [environment group](/environment-management/environment-groups/overview))
...
- _[Archive](/deployment/deployment-manager#application-archives)_ - stores the application package itself, **Upload** it from your local machine (Local File) or via any external link (URL)
- _[Git / SVN](/deployment/deployment-manager#git--svn-projects)_ - saves the access credentials to your projects at the remote Git / SVN repositories; click the **Add Repo** button and specify the required details

Once your package is added to the Deployment Manager, it can be [automatically deployed](/deployment/deployment-guide) to the required environment by following the linked guide.
...
If a [collaborator](/account-and-pricing/accounts-collaboration/collaboration-overview) works on the account, the icon for the appropriate actions is automatically customized to simplify tasks analysis. Hover over such a custom icon to view the email address of the corresponding account.
...
1. The **Account** section allows managing [two-factor authentication](/account-and-pricing/two-factor-authentication) for your account, as well as changing the password.

![Locale Dropdown](./img/DashboardGuide/41-account-user-settings.png)

2. Within the **_Access Tokens_** tab, you can configure [personal access tokens](/account-and-pricing/personal-access-tokens) for your account.
...
- **Public Keys** - stores [public keys](/deployment-tools/ssh/add-ssh-key) added to the platform (required for remote access via local SSH client)
- **Private Keys** - lists [private keys](/deployment/ssh-access-to-git-repository) added to the platform (required for access to your private Git repository over SSH)
- **SSH Connection** - shows the required steps to connect to your account via [SSH Gate](/deployment-tools/ssh/ssh-access/ssh-gate) and allows accessing particular nodes directly in the browser using [Web SSH](/deployment-tools/ssh/ssh-access/web-ssh).
- **SFTP / Direct SSH Access** - displays connection data for [SFTP/FISH protocols](/deployment-tools/ssh/ssh-protocols).
...
For a detailed overview of the [Account Collaboration](/account-and-pricing/accounts-collaboration/collaboration-overview) feature, refer to the linked guide.

## Upgrade Trial Account & Balance

Depending on the [account type](/account-and-pricing/account-types) (trial or billing), either the **Upgrade Account** or **Balance** section is displayed at the top of the dashboard panel.
...
- Use the **Upgrade Account** button to get a fully functional [account with no limitations](/account-and-pricing/account-types#billing).
- The **Learn about Trial Limitations** option opens the appropriate [Account Limits](/account-and-pricing/quotas-system) tab within the _Quotas & Pricing frame_.
- Click on **Learn about Pricing** to be redirected to the documentation page with information about the [pricing model](/account-and-pricing/pricing-model-overview).
- Select the **See statistics on recent resource usage** option to open the account’s [billing history](/account-and-pricing/resource-charging/monitoring-consumed-resources#billing-history).
...
- Click on **Refill Balance** to [submit the payment](/account-and-pricing/upgrade-&-refill-account).
- **Configure auto-refill** to enable [automatic refill](/account-and-pricing/upgrade-&-refill-account) of the account balance (based on the following conditions: _Weekly, Monthly or when Balance less than specified sum_).
- **Payment Methods** option gives an opportunity to choose the default payment method for the account or add a new one.
- Click on the **Quotas & Pricing** item to see the [information frame](/account-and-pricing/resource-charging/pricing-faq#how-much-do-resources-cost) with a set of tabs about platform _Regions_ (if multiple ones are available)_, Pricing, and Account Limits_.
- **Billing History** shows [account spends](/account-and-pricing/resource-charging/monitoring-consumed-resources#billing-history) for the specified period.
...
- **Tutorial** starts a short, [interactive guide](/quickstart/welcome-tutorial), which explains the basics of working with the platform
