...
1. To change the default behavior, which allows all connections, you need to comment/remove the appropriate line, as it is shown in the image below (e.g. using the [file configuration manager](/application-setting/configuration-file-manager) or via [Web SSH](/deployment-tools/ssh/ssh-access/web-ssh)):
...
- if your database node has _[public IP](/application-setting/external-access-to-applications/public-ip)_
