...
If required, change the **Environment Name** and destination [Region](/environment-management/environment-regions/choosing-a-region). Once installation is completed you have to take care about security of nodes communication with the help of authentication key file.
...
1. Log into one of database nodes via [Web SSH](/deployment-tools/ssh/ssh-access/web-ssh).
...
- Click on the **Config** button next to your database nodes in order to access [File Manager](/application-setting/configuration-file-manager).
...
4. Apache requires a special module for being able to [interact with the MongoDB server](/database/mongodb/connection-to-applications/php-connection-to-mongodb), so you need to add it within configs.
