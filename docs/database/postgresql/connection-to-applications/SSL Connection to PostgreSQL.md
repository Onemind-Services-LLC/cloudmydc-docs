...
1. To start with, connect to your database server via [SSH Gate](/deployment-tools/ssh/ssh-overview).
...
- we won’t explain commands parameters in details here, but if you’d like to know more, just refer to the [Self-Signed Custom SSL](/application-setting/ssl/self-signed-custom-ssl) page in our documentation or check the official [OpenSSL](https://www.openssl.org/docs/manmaster/man1/openssl.html) site for the full list of available actions
- you can also use [custom SSL](/application-setting/ssl/custom-ssl) certificate similarly to the described below (follow the Generate a Custom SSL Certificate section of the linked guide to get such). In this latter case, you can skip the generation instruction and jump directly to the [6th step](/database/postgresql/connection-to-applications/ssl-connection-to-postgresql#postgresql-server-configuration) of this instruction
...
3. After the files - postgresql.key, postgresql.crt, root.crt - are ready, you need to move them to to the .**postgresql** folder at your client machine (for that, you can use [FTP add-on](/deployment-tools/ftp-ftps-support) or just copy and paste files content).
...
1. In order to connect to the DB server via SSL, you need either [public IP](/application-setting/external-access-to-applications/public-ip) or [endpoint](/application-setting/external-access-to-applications/endpoints) being attached for your PostgreSQL database container.
...
Now you can connect your application to database (use the [Connect to Database](/database/database-hosting/database-connection-strings) guide as an example) and enable SSL configurations for your project to encrypt your data while fetching/transferring.
