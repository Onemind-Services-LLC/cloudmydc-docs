...
2. Within the opened [configuration file manager](/application-setting/configuration-file-manager) panel, navigate to the **/etc/nginx/** **_nginx.conf_** file. Find the **_include app_servers_** strings:
...
5. Access your node via SSH (e.g. using [Web SHH](/deployment-tools/ssh/ssh-access/web-ssh)) and run the selected module with the appropriate command executed from the project directory:
