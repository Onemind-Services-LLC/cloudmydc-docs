...
- you can automate horizontal scaling based on incoming load with the help of [tunable triggers](/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling)
...
- **mount points** - custom [mounts](/docs/Data%20Storage%20Container/Data%20Sharing/Mount%20Points) will be moved only if the appropriate [volume](/docs/Container/Container%20Configuration/Volumes) is configured
- **add-ons** - any [add-ons](/docs/Deployment%20Tools/Cloud%20Scripting%20&%20JPS/Marketplace) installed on the layer won’t be available
...
The maximum number of the same-type servers within a single environment layer depends on a particular hosting provider settings (usually this limit stands for 16 nodes). You can check the exact value within the **_Quotas & Pricing > [Account Limits](/docs/Account&Pricing/Quotas%20System)_** information frame.
...
Having several same-type nodes within a layer enables their synchronous management. Thus, all containers can be simultaneously [configured](/docs/Container/Container%20Configuration/Configuration%20Tools), inspected for logs and statistics, [restarted or redeployed](/docs/Container/Container%20Redeploy) through the corresponding icons.
...
More information on this labeling feature can be found in the [Environment Aliases](/docs/EnvironmentManagement/Environment%20Aliases) document.
...
Now, you know how easy it is to horizontally scale instances in the platform and aware of the operation specifics. Also, feel free to configure an [automatic nodes scaling](/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling) to smoothly overcome high load spikes without overpaying for unused resources.
...
3. At the bottom of the frame, a redirect to the [Automatic Horizontal Scaling](/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling) section can be found.
