...
- [PHP Versions](/docs/php/php-dev-center#php-versions)
- [PHP Application Servers](/docs/php/php-dev-center#php-application-servers)
- [PHP Application Deployment](/docs/php/php-dev-center#php-application-deployment)
- [Domains Management](/docs/php/php-dev-center#domains-management)
- [automatic vertical scaling](/docs/php/php-dev-center#automatic-vertical-scaling)
- [Manual Horizontal Scaling](/docs/php/php-dev-center#manual-horizontal-scaling)
- [Automatic Horizontal Scaling](/docs/php/php-dev-center#automatic-horizontal-scaling)
- [PHP Clustering](/docs/php/php-dev-center#php-clustering)
- [Database Connection to PHP Application](/docs/php/php-dev-center#database-connection-to-php-application)
- [Modules and Accelerators](/docs/php/php-dev-center#modules-and-accelerators)
- [PHP Security](/docs/php/php-dev-center#php-security)
...
You can choose the engine version you need while environment creation and easily switch between them afterward via intuitive [platform UI](/docs/php/php-versions).
...
- [Apache](/docs/php/php-app-servers/apache-php/apache-php)
- [NGINX](/docs/php/php-app-servers/nginx-php/nginx-php)

To see in detail how these servers can be configured, and find out the list of editable config files for each of them, refer to the document [PHP App Server Configuration](/docs/php/php-app-server-configuration).
...
[Zero downtime deployment](/docs/php/zdt-deployment-for-php) provides the ability to run any cloud-native or legacy application. In order to deliver your PHP projects to the created environment within the platform, you can choose one of the available [deployment options](/docs/deployment/deployment-guide): via Archive/URL, GIT/SVN, Bitbucket project, etc.

Besides, you can configure an [automatic periodic re-deployment](/docs/deployment/git-&-svn-auto-deploy/auto-deploy-overview) from the repository (it is performed only if new code changes appear).
...
Also, all platform-certified PHP nodes are provided with the pre-installed [**Composer**](/docs/php/composer-dependency-manager) dependency manager to quickly install all the required packages for the deployed project and keep them up-to-date.
...
Using [multiple domains](/docs/application-setting/domain-name-management/multiple-domain-for-php) on a single PHP server increases the application usability, efficiency, and scalability, simultaneously saving the costs on the necessity to set up separate instances.
...
In order to set up automatic resource consumption by your PHP server, open the environment topology wizard and specify the upper scaling limit of [cloudlets](/docs/platform-overview/cloudlet) (each equals to 128 MiB and 400 MHz) with the slider located at the top:
...
Also, as you can see, when you add extra servers, [[load balancing]](/docs/load-balancers/load-balancing) is enabled automatically.

Please, find more details about manual [Horizontal Scaling](/docs/application-setting/scaling-and-clustering/horizontal-scaling) in the documentation.
...
Implementing the [PHP sessions clustering](/docs/php/php-sessions-clustering) for your application can highly increase its availability and failover capabilities using the sticky sessions, run on several app server nodes. This mechanism is ensured by a Memcached node that should be added to your environment and used as storage for the processed sessions’ backups.

While scaling out PHP server with your deployed application, you get its exact copy with all the same configurations and files inside due to built-in [data synchronization](/docs/application-setting/scaling-and-clustering/data-synchronization).
...
- [MySQL/MariaDB Connection](/docs/database/mysql-mariadb-percona/connection-to-applications/php-connection)
- [PostgreSQL Connection](/docs/database/postgresql/connection-to-applications/php-connection-to-postgresql)
- [MongoDB Connection](/docs/database/mongodb/connection-to-applications/php-connection-to-mongodb)

## Modules and Accelerators

While working with the platform, you can extend the functionality of the environment through setting different PHP extensions at the appropriate application servers. You can either select one of the default extensions and activate it, or upload your own custom one - both these operations are described in the [PHP Modules](/docs/php/php-apps-specifications/php-extensions) instruction.

As a separate kind of PHP extension, [PHP accelerators](/docs/php/php-apps-specifications/php-accelerators) can be determined. They are used for enlarging your application efficiency through caching the already compiled parts of PHP code, which can be quickly executed upon a user’s request.

Apache PHP and NGINX servers include a particular [set of the inbuilt modules](/docs/php/php-app-servers/apache-php/apache-modules). In addition, there are a few separate tutorials on configuring the various modules:

- [Adding Custom Apache Modules](/docs/php/php-app-servers/apache-php/add-apache-modules)
- [Apache WebDav Module](/docs/php/php-app-servers/apache-php/apache-webdav-module)
- [Apache Statistics Module](/docs/php/php-app-servers/apache-php/apache-statistics-module)
- [NGINX WebDav Module](/docs/php/php-app-servers/nginx-php/nginx-webdav-module)
- [New Relic Monitoring](/docs/application-setting/monitoring-with-new-relic/new-relic-installation)

## PHP Security

[SSL / TLS](/docs/application-setting/ssl/secure-sockets-layer) (Secure Sockets Layer / Transport Layer Security) is the industry standard security technology for establishing an encrypted connection between a web server and a browser. This technology is supported by the platform and ensures that all passed data remains private and confidential, i.e., it can’t be intercepted by a third party.

Besides the common protection methods, security of your application can be increased through [customizing the main PHP configuration file](/docs/php/php.ini-security-settings), named **_php.ini_**. You can edit a number of default settings it contains or adds your own in accordance with the needs of your application.

Additional configuration guidance is available for particular application servers:

- [Apache Security Configurations](/docs/php/php-app-servers/apache-php/apache-security-configurations)
- [NGINX Security Configurations](/docs/php/php-app-servers/nginx-php/nginx-security-configurations)
