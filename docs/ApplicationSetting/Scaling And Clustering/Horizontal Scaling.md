...
With the platform, hosting of your applications becomes truly flexible. In addition to [automatic vertical scaling](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Vertical%20Scaling), the platform also lets you increase/decrease the number of servers in your environment manually or [automatically](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling).
...
- you can automate horizontal scaling based on incoming load with the help of [tunable triggers](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling)
...
- in case of scaling in (i.e decreasing the nodes number), the last container added to the layer is the first one to be removed (unless it is [selected explicitly](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Horizontal%20Scaling#managing-nodes-within-layer))

:::

Next, you can select the required [scaling mode](/docs/ApplicationSetting/Scaling%20And%20Clustering/Horizontal%20Scaling) from the appropriate drop-down list. Also, for additional details refer to the [horizontal scaling specifics](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling) section below.
...
- **mount points** - custom [mounts](http://localhost:3000/docs/Data%20Storage%20Container/Data%20Sharing/Mount%20Points) will be moved only if the appropriate [volume](http://localhost:3000/docs/Container/Container%20Configuration/Volumes) is configured
- **add-ons** - any [add-ons](http://localhost:3000/docs/Deployment%20Tools/Cloud%20Scripting%20&%20JPS/Marketplace) installed on the layer won’t be available
...
The maximum number of the same-type servers within a single environment layer depends on a particular hosting provider settings (usually this limit stands for 16 nodes). You can check the exact value within the **_Quotas & Pricing > [Account Limits](http://localhost:3000/docs/Account&Pricing/Quotas%20System)_** information frame.
...
Having several same-type nodes within a layer enables their synchronous management. Thus, all containers can be simultaneously [configured](http://localhost:3000/docs/Container/Container%20Configuration/Configuration%20Tools), inspected for logs and statistics, [restarted or redeployed](http://localhost:3000/docs/Container/Container%20Redeploy) through the corresponding icons.
...
More information on this labeling feature can be found in the [Environment Aliases](http://localhost:3000/docs/EnvironmentManagement/Environment%20Aliases) document.
...
Now, you know how easy it is to horizontally scale instances in the platform and aware of the operation specifics. Also, feel free to configure an [automatic nodes scaling](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling) to smoothly overcome high load spikes without overpaying for unused resources.
...
- In case a [**high availability**](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Session%20Replication%20for%20HA) option is enabled for the layer (available for the _Tomcat_ and _TomEE_ application servers only), the nodes are added and removed in pairs.
...
3. At the bottom of the frame, a redirect to the [Automatic Horizontal Scaling](http://localhost:3000/docs/ApplicationSetting/Scaling%20And%20Clustering/Automatic%20Horizontal%20Scaling) section can be found.
