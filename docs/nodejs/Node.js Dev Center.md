...
- [Node.js Process Managers](/nodejs/node.js-dev-center#nodejs-process-managers)
- [Domains Management](/nodejs/node.js-dev-center#domains-management)
- [Automatic Vertical Scaling](/nodejs/node.js-dev-center#automatic-vertical-scaling)
- [Manual Horizontal Scaling](/nodejs/node.js-dev-center#manual-horizontal-scaling)
- [Automatic Horizontal Scaling](/nodejs/node.js-dev-center#automatic-horizontal-scaling)

## Node.js Environment Hosting

To run your Node.js application, you need to [set up](/environment-management/setting-up-environment#choosing-programming-language-or-specific-deployment-option) the appropriate environment using the powerful and intuitive **Topology Wizard**. Switch to the **_Node.js_** tab, select the required engine version for your application server and add any other [software stack](/quickstart/software-stack-versions) required. If needed, adjust other parameters, such as cloudlets and nodes count, Public IPv4 and IPv6, etc.
...
Usually, due to the [ports auto-redirect](/container/container-configuration/ports#ports-auto-redirect) algorithm, the deployed projects are instantly accessible without any additional manipulations. Herewith, for greater accuracy, you can manually exclude some of the services from the auto-redirect search by listing the appropriate ports via the **_REDIRECT_EXCLUDE_PORTS_** [variable](/container/container-configuration/variables).
...
By default, the npm package manager is used for the NodeJS application servers, but, if necessary, it can be easily switched to yarn. For that, access the [container variables](/container/container-configuration/variables) frame and set the appropriate **_PACKAGE_MANAGER_** value (either _npm_ or _yarn_).
...
Users can choose the necessary process manager during container [redeployment](/category/container-deployment) or by adjusting the **_PROCESS_MANAGER_** [variable](/container/container-configuration/variables) (using _forever_, _npm_, _pm2_, or _supervisor_ as value, container restart is needed to apply new options).
