...
Additionally, the feature allows developers to isolate [groups of environments](https://cloudmydc.com/) within a particular account. Just turn on the **_Network Isolation_** switcher in the **Add/Edit Group** frame.
...
Summing all this up, _Network Isolation_ is a useful and user-oriented feature aimed to prevent undesired access to your environments. Commonly, it’s a good practice to isolate your [applications](https://cloudmydc.com/) from each other. For example:
...
- If you are [cloning](https://cloudmydc.com/) an initially isolated project, it will be protected from the clone’s influence (e.g. if your copied project inherited a “hardcoded” database access, it will be disabled by the network Isolation feature so that the actual production data could not be changed)
